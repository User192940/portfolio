...
$size = 2048000; // 2048000/1024/1000 = 2MB
  ...
  if (!isset($imageError)) {
    ...
    if (isset($image_id)) {
      $sql = 'INSERT INTO php_blog_pages (image_id, title, slug, created)
              VALUES(?, ?, ?, NOW())';
      $stmt->prepare($sql);
      $stmt->bind_param('iss', $image_id, $_POST['title'], $_POST['slug']);
    } else {
      // create SQL
      $sql = 'INSERT INTO php_blog_pages (title, slug, created)
              VALUES(?, ?, NOW())';
      $stmt->prepare($sql);
      $stmt->bind_param('ss', $_POST['title'], $_POST['slug']);
    }
...
$nav = "blog-admin";
$title_section = "Blog: Admin - Page Insert";
...
?>
        ...
        <li <?php if(isset($errorArticle)) echo 'class="error"'?>>
            ...
            <textarea name="slug" cols="60" rows="8" class="widebox" id="slug"><?php if (isset($error)) {
