...
if (isset($_GET['page_id']) && !$_POST) {
    // prepare SQL query
    $sql = 'SELECT page_id, image_id, title, slug
            FROM php_blog_pages WHERE page_id = ?';
    if ($stmt->prepare($sql)) {
        ...
        $stmt->bind_result($page_id, $image_id, $title, $slug);
        $stmt->fetch();
...
$size = 2048000; // 2048000/1024/1000 = 2MB
  ...
  if (!isset($imageError)) {
    ...
    if (isset($image_id)) {
      $sql = 'UPDATE php_blog_pages SET image_id = ?, title = ?, slug = ?
              WHERE page_id = ?';
      $stmt->prepare($sql);
      $stmt->bind_param('issi', $image_id, $_POST['title'], $_POST['slug'], $_POST['page_id']);
    } else {
      // create SQL
      $sql = 'UPDATE php_blog_pages SET image_id = NULL, title = ?, slug = ?
              WHERE page_id = ?';
      $stmt->prepare($sql);
      $stmt->bind_param('ssi', $_POST['title'], $_POST['slug'], $_POST['page_id']);
    }
...
?>
        ...
        <li <?php if(isset($errorArticle)) echo 'class="error"'?>>
            ...
            <textarea name="slug" cols="60" rows="8" class="widebox" id="slug"><?php 
                            if (isset($error)) {
                                echo htmlentities($_POST['slug'], ENT_COMPAT, 'utf-8');
                            } else {
                                echo htmlentities($slug, ENT_COMPAT, 'utf-8');
                            }
                            ?></textarea>
        </li>
        <li>
